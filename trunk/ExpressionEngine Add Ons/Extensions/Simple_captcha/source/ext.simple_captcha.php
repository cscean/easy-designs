...
/*
=====================================================
 Simple Captcha Challenges - by Easy! Designs, LLC
-----------------------------------------------------
...
class Simple_captcha {
  ...
  var $name            = 'Simple Captcha Challenges';
  var $version         = '0.1';
   ...
   * Constructor 
   */
  function Simple_captcha($settings='') 
  {
  ...
  function settings_form($current) 
  {
    ...
    $DSP->body = $DSP->form_open(
                   array(
                       ...
                       'name'   => 'simple_captcha_settings',
                       'id'     => 'simple_captcha_settings'
                       ),
    ...
    if (empty($current)) {
      $challenges = array(
                      array('q' => $LANG->line('example_question'),
  ...
  function save_settings()
  {
    ...
    foreach ($_POST as $k => $v) {
      if ($k[0] == 'q') {
		...
		$sql =  $DB->update_string($PREFS->ini('db_prefix').'_extensions', 
              array( 'settings' => addslashes(serialize($this->settings)) ), 
              array( 'class' => 'Simple_captcha' ) 
            );
  ...
  function create_captcha($old_word = '') 
  {
    ...
    if ($question === '') {
      ...
      $DB->query("INSERT INTO ".$PREFS->ini('db_prefix')."_captcha (date, ip_address, word) VALUES (UNIX_TIMESTAMP(), '".$IN->IP."', '".$DB->escape_str($challenge['a'])."')");
...
/* End of file ext.simple_captcha.php */
/* Location: ./system/extensions/ext.simple_captcha.php */